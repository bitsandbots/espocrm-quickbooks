define("modules/quick-books/views/admin/integrations/quick-books", ["exports", "views/admin/integrations/edit"], function (_exports, _edit) {
  "use strict";
   ...
   */
  class QuickBooksIntegrationView extends _edit.default {
    events = {
    ...
    renderStatusSection() {
      ...
      const connectionHtml = isConnected ? `<span style="color:#2b7de9;font-weight:600">&#10003; Connected</span>
               ...
               <span style="color:#999;margin-left:8px;font-size:12px">${connectedAt}</span>` : `<span style="color:#999">Not connected to QuickBooks</span>`;
    ...
    actionConnectQuickBooks() {
      ...
      Espo.Ajax.postRequest('QuickBooksIntegration/initOAuth', {}).then(data => this.openOAuthPopup(clientId, siteUrl, data.state)).catch(() => Espo.Ui.error('Could not initiate QuickBooks OAuth. Check server logs.'));
    }
    openOAuthPopup(clientId, siteUrl, state) {
      const redirectUri = `${siteUrl}/?entryPoint=QuickBooksOauthCallback`;
      const authUrl = 'https://appcenter.intuit.com/connect/oauth2' + '?client_id=' + encodeURIComponent(clientId) + '&scope=com.intuit.quickbooks.accounting' + '&redirect_uri=' + encodeURIComponent(redirectUri) + '&response_type=code' + '&state=' + encodeURIComponent(state);
    ...
    actionRunSync() {
      ...
      Espo.Ajax.postRequest('QuickBooksIntegration/runSync', {}).then(() => {
  ...
  _exports.default = QuickBooksIntegrationView;
});
//# sourceMappingURL=quick-books.js.map ;